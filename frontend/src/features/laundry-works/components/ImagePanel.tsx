    ...
    <section className="rounded-[22px] border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          {viewAllAction ? (
            <button type="button" onClick={viewAllAction.onClick} disabled={viewAllAction.disabled || viewAllAction.loading} className="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-blue-900 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50">
              {viewAllAction.loading ? 'กำลังโหลด...' : viewAllAction.label || 'ดูทั้งหมด'}
            </button>
          ) : null}
          {canUploadImage ? (
            <button type="button" onClick={onUploadImage} disabled={uploadAction?.disabled || uploadAction?.loading} className="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-blue-900 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50">
      ...
      {images.length ? (
        <div className="mt-5 grid grid-cols-3 gap-3 sm:grid-cols-4 lg:grid-cols-3 xl:grid-cols-4">
          {images.map((image, index) => {
            const imageUrl = image.thumbnailUrl || image.url
            return (
              <figure key={image.id || index} className="overflow-hidden rounded-2xl border border-slate-200 bg-slate-50">
                {imageUrl ? (
                ...
                {image.name || image.caption || image.uploadedAt ? (
                  <figcaption className="space-y-1 px-2.5 py-2 text-xs text-slate-500">
