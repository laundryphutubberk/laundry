...
export function WorkHeader({ work, status, workspace, meta, actions = [], loading = false, error = null }: WorkHeaderProps) {
  ...
  if (error) {
    return (
      <section className="rounded-[22px] border border-red-100 bg-red-50 p-5 text-red-800 shadow-sm">
  ...
  return (
    <section className="rounded-[22px] border border-slate-200 bg-white p-6 shadow-sm md:p-7">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-3">
            <h1 className="truncate text-2xl font-bold tracking-tight text-slate-950 md:text-[32px] md:leading-tight">
              งาน #{title}
            </h1>
          ...
          {safeActions.map((action, index) => {
            ...
            return (
              <button
                ...
                className={
                  primary
                    ? 'rounded-xl bg-blue-900 px-5 py-3 text-sm font-semibold text-white shadow-sm hover:bg-blue-800 disabled:cursor-not-allowed disabled:opacity-50'
                    : 'rounded-xl border border-slate-200 bg-white px-5 py-3 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50'
                }
              >
