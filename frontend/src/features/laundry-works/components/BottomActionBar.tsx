...
export function BottomActionBar({ actions = {}, state = {}, loading = false, error = null }: BottomActionBarProps) {
  ...
  return (
    <div className="sticky bottom-0 z-20 border-t border-slate-200 bg-white/95 px-4 py-4 shadow-[0_-14px_34px_rgba(15,23,42,0.10)] backdrop-blur supports-[backdrop-filter]:bg-white/85">
      <div className="mx-auto flex w-full max-w-screen-2xl flex-col-reverse gap-3 md:flex-row md:items-center md:justify-between md:px-4 lg:px-8">
        <button
          ...
          className="rounded-2xl border border-slate-200 bg-white px-7 py-3.5 text-sm font-bold text-slate-700 shadow-sm hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50"
        >
          {backAction.loading ? 'กำลังโหลด...' : backAction.label || 'ย้อนกลับ'}
        </button>

        <div className="flex flex-col gap-2 md:flex-row md:items-center">
          {error ? <p className="rounded-2xl bg-red-50 px-4 py-2 text-sm font-semibold text-red-600">{error}</p> : null}
          ...
          {actions.destructive ? (
            <button
              ...
              className="rounded-2xl border border-red-200 bg-white px-7 py-3.5 text-sm font-bold text-red-700 shadow-sm hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50"
            >
          ...
          {canContinue ? (
            <button
              ...
              className="rounded-2xl bg-blue-900 px-8 py-3.5 text-sm font-black text-white shadow-lg shadow-blue-900/15 hover:bg-blue-800 disabled:cursor-not-allowed disabled:opacity-50"
            >
