  ...
  return (
    <section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-950">ปัญหา/หมายเหตุ</h2>
          <p className="mt-1 text-sm text-slate-500">แสดงปัญหาที่ต้องเห็นก่อนดำเนินงานต่อ</p>
        </div>
        {canCreateIssue ? (
          <button
            type="button"
            ...
            className="rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-blue-900 shadow-sm hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50"
          >
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="rounded-full bg-red-500 px-2 py-0.5 text-[11px] font-bold uppercase tracking-wide text-white">Issue</span>
                      <p className="text-sm font-bold text-slate-950">{issue.title || issue.issueType || issue.type || 'ปัญหา'}</p>
                    </div>
                    <div className="mt-2 grid gap-1 text-xs text-slate-500 sm:grid-cols-2">
                      {issue.quantity !== undefined && issue.quantity !== null ? <span>จำนวน: <strong className="text-slate-700">{issue.quantity}</strong></span> : null}
                      {issue.itemTypeName ? <span>รายการ: <strong className="text-slate-700">{issue.itemTypeName}</strong></span> : null}
                      {issue.reportedAt ? <span>เวลา: <strong className="text-slate-700">{issue.reportedAt}</strong></span> : null}
                      {issue.reportedBy ? <span>ผู้รายงาน: <strong className="text-slate-700">{issue.reportedBy}</strong></span> : null}
                    </div>
                    {issue.description ? <p className="mt-3 text-sm leading-6 text-slate-600">{issue.description}</p> : null}
                  </div>
                  <span className={`shrink-0 rounded-full px-3 py-1 text-xs font-bold ring-1 ${statusTone}`}>{status}</span>
                </div>
              </article>
            )
          })}
